...
3. **Prefer Cortex Code native tools when available:** run every query below via `sql_execute` instead of `snowflake_utils.execute_query()`. Fall back to the plugin helper under Claude Code.
